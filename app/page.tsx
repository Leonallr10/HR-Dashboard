...
export default function HomePage() {
  ...
  const topDepartment = availableDepartments.reduce(
    (top, dept) => {
      const deptEmployees = employees.filter((emp) => emp.company.department === dept)
      const deptAvg = deptEmployees.reduce((sum, emp) => sum + emp.performanceRating, 0) / deptEmployees.length
      return deptAvg > (top.avg || 0) ? { name: dept, avg: deptAvg } : top
    },
    { name: "", avg: 0 },
  )
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-12 text-center">
          ...
          <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
            Streamline HR operations with intelligent performance tracking, comprehensive analytics, and seamless team
            management
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            ...
            <CardContent className="p-6 relative">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-blue-200 text-xs mt-1">Performance score</p>
                </div>
          ...
          <Card className="border-0 shadow-lg bg-gradient-to-br from-orange-500 to-orange-600 text-white overflow-hidden relative">
            ...
            <CardContent className="p-6 relative">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-orange-100 text-sm font-medium mb-1">Top Department</p>
                  <p className="text-lg font-bold truncate">{topDepartment.name || "N/A"}</p>
                  <p className="text-orange-200 text-xs mt-1">{topDepartment.avg.toFixed(1)} avg rating</p>
                </div>
